...
export async function provisionUser(params: ProvisionParams): Promise<User> {
  ...
  try {
    ...
    if (settings?.agent_name || settings?.custom_instructions) {
      ...
      parts.push(`\n## Communication During Tasks\nWhen a task will take more than a few seconds (browsing, building, installing, researching, etc.):\n1. IMMEDIATELY send a short message BEFORE starting work: "On it, give me a moment." or "Looking into that now." or "Working on it, this will take a minute."\n2. If the task has multiple steps, send brief updates as you go: "Found the info, putting it together now." or "Site is building, almost ready."\n3. NEVER go silent for a long time and then dump everything at once. The user will think you're broken.\n4. Keep status messages short — one sentence max. Save the detail for the final response.\n5. If something fails and you need to retry, tell the user: "First approach didn't work, trying another way." Don't silently retry 5 times.`);
      ...
      parts.push(`\n## Web Preview\nWhen you build websites or web apps, always start the dev server on port 8080 (use \`--port 8080\` or equivalent). The user can preview it live at the URL in the PREVIEW_URL environment variable. Tell the user this URL when you start a dev server.\n\nIMPORTANT RULES:\n- After starting a dev server, send the preview link to ALL connected messaging apps (Telegram, WhatsApp, Discord, Slack). Format: "Your website preview is ready: [URL]"\n- When the user asks to "preview" or "see" the site, just give them the PREVIEW_URL. Do NOT use the browser tool to open the external preview URL — it goes through Cloudflare/DNS and may be slow or error out.\n- If YOU need to verify the site is working, use \`curl -s http://localhost:8080\` or the browser tool with \`http://localhost:8080\` (local access, not the external URL).\n- If curl or browser shows a build error, fix it. But if the dev server is running and responding, just give the user the PREVIEW_URL and let them check it in their browser.\n- Do NOT enter a debugging loop trying to make the external preview URL work. DNS and SSL can take a moment. The site works if localhost:8080 responds.`);

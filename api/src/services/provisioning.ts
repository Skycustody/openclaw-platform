...
export async function provisionUser(params: ProvisionParams): Promise<User> {
  ...
  try {
    ...
    if (settings?.agent_name || settings?.custom_instructions) {
      ...
      parts.push(`\n## Web Preview — CRITICAL\nWhen you build websites or web apps, start the dev server on port 8080.\n\nThe user's preview link is in the PREVIEW_URL environment variable. Run \`echo $PREVIEW_URL\` to get it.\n\nRULES:\n- ALWAYS send the user the PREVIEW_URL link (e.g. https://xxx.valnaa.com). NEVER send localhost or 127.0.0.1 links — the user cannot access localhost, they are on a different computer.\n- After starting a dev server, immediately send the PREVIEW_URL to the user and all connected messaging apps.\n- For YOUR OWN testing, use localhost:8080. For the USER, always use PREVIEW_URL.\n- Do NOT debug the external URL if it's slow. If localhost:8080 works, the site is fine — tell the user the PREVIEW_URL and let them open it.`);

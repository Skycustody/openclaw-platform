...
export async function injectApiKeys(
  ...
  try {
    ...
    // Clear all managed bindings (both legacy {channel} and new {match:{channel}} formats)
    const managed = ['telegram', 'discord', 'slack', 'whatsapp', 'signal'];
    config.bindings = (config.bindings || []).filter((b: any) => {
      const ch = b.match?.channel ?? b.channel;
      return !ch || !managed.some(t => ch === t || String(ch).startsWith(`${t}-`));
    });
